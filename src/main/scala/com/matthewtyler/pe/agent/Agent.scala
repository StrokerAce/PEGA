...
class Agent(agentType : String, 
  ...
  lazy val agentString = "%s %s".format(agentType,id)
  
  /**
   * Implement Instruction trait
   */
  def apply(myState : PEState,opponentState : PEState) = strategy.apply(myState,PEState.toFrameOfReference(myState,opponentState))
  ...
  def getScore(myCurrentState         : PEState,
    ...
    constraints.updateStateWithScore(myCurrentState,myPreviousState,opponentCurrentState,opponentPreviousState,this.agentType match {
   ...
   */
  def mate(p : Agent) = new Agent(agentType,Agent.nextId,constraints,this.strategy mate p.strategy)
