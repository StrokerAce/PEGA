...
object AgentCompetitionCanvas extends Panel with Logging with Publisher{
  ...
  case class AgentTrace(traceColor : Color, states : List[PEState],dataXOffset : Int) {
    ...
    def addState(newState : PEState) = states match {
      ...
      case h :: t => AgentTrace(traceColor,newState :: states,dataXOffset)
    }
    ...
    def paint(g : Graphics) = {
      ...
      val (score,energy,speed,heading,winString) = states match {
        case h :: t => (h.score,h.energy,h.velocity.magnitude,h.velocity.theta,if(h.winningState) "*** WINNER! ***" else "")
  ...
  class CompetitionActor(private val result : PECompetitionResult) extends Actor {
    ...
    override def preStart = {
      ...
      AgentCompetitionCanvas.evaderTrace = AgentTrace(Color.BLUE,List(),evaderXOffset)
      AgentCompetitionCanvas.pursuerTrace = AgentTrace(Color.RED,List(),pursuerXOffset)      
    }
  ...
  private var evaderTrace = AgentTrace(Color.BLUE,List(),evaderXOffset)
  private var pursuerTrace = AgentTrace(Color.RED,List(),pursuerXOffset)
  ...
  def start(competitionResult : PECompetitionResult) = {
    ...
    xyRatio = max((maxX/xOffset),(maxY/yOffset))
