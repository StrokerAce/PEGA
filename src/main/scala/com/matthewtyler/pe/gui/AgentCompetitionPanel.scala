...
object AgentCompetitionPanel extends BorderPanel with Logging {
  ...
  //Agent/fitness pairs.
  val evaders = HashMap.empty[Int, List[AgentScore]]
  val pursuers = HashMap.empty[Int, List[AgentScore]]
  ...
  var competitionResult = PECompetitionResult(List.empty[PEState], List.empty[PEState], 0, 0, 0, 0)
  ...
  reactions += {
    ...
    case summary: PETournamentResultSummary =>
      ...
      evaders.put(summary.generation, evaderScores.toList.sorted)
      pursuers.put(summary.generation, pursuerScores.toList.sorted)
