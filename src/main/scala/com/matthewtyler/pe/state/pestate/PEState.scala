...
case class PEState(val position: Vector,
  ...
  private lazy val peStateString = s"State\nEnergy $energy\nPosition\n$position\nVelocity\n$velocity"
  ...
  def nextState(deltaRotation: Double, deltaSpeed: Double) = {
    ...
    if (MathHelper.doubleEquals(energy, 0.0)) {
      ...
      if (!isActive) {
        ...
        debug(s"$agent is out of energy. Speed will decay by $decaySpeed")
      ...
      if (energyToAccelerate > energy) {
        throw new RuntimeException(s"Energy to accelerate by $deltaSpeed ($energyToAccelerate) is greater than remainingEnergy $energy")
      }
